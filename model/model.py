...
class JRTransformer(nn.Module):
	...
	def forward(self, x_joint):
		...
		# p1 = x_external.detach()
		# p1 = p1.cpu()
		# p1 = p1.numpy()
		# p1 = p1[0,:13,0]
		# p2 = x_external.detach()
		# p2 = p2.cpu()
		# p2 = p2.numpy()
		# p2 = p2[0, 13:, 0]
		# attention_matrix_1 = np.outer(p1,p1)
		# attention_matrix_2 = np.outer(p1, p2)
		# attention_matrix_3 = np.outer(p2, p1)
		# attention_matrix_4 = np.outer(p2, p2)
		#
	    # #创建热力图
		# plt.figure(figsize=(5, 5))  # 可能需要调整尺寸以适应13个关节点
		# sns.heatmap(attention_matrix_1, annot=False, cmap='hot', square=False,vmin =0)
		# sns.heatmap(attention_matrix_2, annot=False, cmap='hot', square=False, vmin=0)
		# sns.heatmap(attention_matrix_3, annot=False, cmap='hot', square=False, vmin=0)
		# sns.heatmap(attention_matrix_4, annot=False, cmap='hot', square=False, vmin=0)
		# p1 = x_external.detach()
		# p1 = p1.cpu()
		# p1 = p1.numpy()
		# p1 = p1[0,:,0]
		...
		# p2 = p2.numpy()
		# p2 = p2[0, :, 0]
		# attention_matrix = np.outer(p1,p2)
		# plt.figure(figsize=(5, 5))
		# sns.heatmap(attention_matrix, annot=False, cmap='viridis', square=False, vmin=0,vmax=2)
		# # 设置坐标轴标签
		# # plt.xlabel('Person 2 ')
		# # plt.ylabel('Person 1 ')
		#
		# # 设置标题
		# # plt.title('Attention Visualization Between Two Person')
		...
		#feat_x_joint =torch.add(x_external,x_internal)
...
#------------------------------relation----------------------------------------
		x_internal = self.Linear3(x_internal)
		feat_x_relation = self.relation_encoder(x_internal)#([128, 26, 128, 128])
		...
		for i in range(len(self.attn_encoder)):
			blk1 = self.attn_encoder[i]
			feat_x_joint = x_external + pe_joint
			feat_x_relation = feat_x_relation + pe_relation
			feat_x_joint, feat_x_relation = blk1(feat_x_joint, feat_x_relation)

		# feat_x_joint.shape: (128,26,128)
		#pred = self.GCN(feat_x_joint).contiguous()
