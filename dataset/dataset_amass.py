...
class AMASSDatasets(Dataset):
    ...
    def __init__(self, path_to_data, skel_path, input_n=16, output_n=14, skip_rate=2, actions=None, split=0):
        """
        :param path_to_data:
        :param actions:
        :param input_n:
        ...
        # full dataset
        amass_splits = [
             ['MPI_Limits', 'TotalCapture', 'Eyes_Japan_Dataset', 'KIT', 'EKUT', 'TCD_handMocap', 'ACCAD', 'CMU' , 'BMLmovi'],
        ...
        for ds in amass_splits[split]:
            ...
            for sub in os.listdir(self.path_to_data + ds):
                ...
                for act in os.listdir(self.path_to_data + ds + '/' + sub):
                    ...
                    # poses[:, 0] = 0
                    # poses += torch.from_numpy(trans[fidxs]).float().cuda()[:, None, :]
